...
// POSIX extensions but are here for simplicities sake. Forward declaration is
// here to avoid exporting sigprocmask when posix is disabled.
int sigprocmask(int, const sigset_t *__restrict, sigset_t *__restrict);
extern "C" {
__attribute__((__returns_twice__)) int __sigsetjmp(sigjmp_buf buffer,
                                                   int        savesigs) {
...
long long
strtoll(const char *__restrict string, char **__restrict end, int base) {
    return mlibc::stringToInteger<long long, char>(string, end, base);
}
unsigned long
strtoul(const char *__restrict string, char **__restrict end, int base) {
    return mlibc::stringToInteger<unsigned long, char>(string, end, base);
}
unsigned long long
strtoull(const char *__restrict string, char **__restrict end, int base) {
...
void *calloc(size_t count, size_t size) {
    ...
    // to prevent overflowing, we divide both sides of the inequality by size
    // and check with that
    ...
    // TODO: this could be done more efficient if the OS gives us already zero'd
    // pages
...
void abort(void) {
    ...
    if (mlibc::sys_sigaction(SIGABRT, &sa, nullptr))
        mlibc::panicLogger()
            << "mlibc: sigaction failed in abort" << frg::endlog;
...
int system(const char *command) {
    ...
    MLIBC_CHECK_OR_ENOSYS(mlibc::sys_fork && mlibc::sys_waitpid &&
                              mlibc::sys_execve && mlibc::sys_sigprocmask &&
                              mlibc::sys_sigaction,
                          -1);
    ...
    if (int e = mlibc::sys_fork(&child)) {
        ...
        while ((err = mlibc::sys_waitpid(child, &status, 0, NULL, &unused)) <
               0) {
            if (err == EINTR)
                continue;
...
int mblen(const char *mbs, size_t mb_limit) {
    ...
    if (auto e = cc->decode_wtranscode(nseq, wseq, mblen_state);
...
int mbtowc(wchar_t *__restrict wc, const char *__restrict mb, size_t max_size) {
    ...
    if (mb) {
        if (*mb) {
            mlibc::code_seq<wchar_t>    wseq{wc, wc + 1};
            mlibc::code_seq<const char> nseq{
                mb, mb + frg::min(max_size, MB_CUR_MAX)};
            ...
            case mlibc::charcode_error::dirty: {
                mlibc::panicLogger()
                    << "decode_wtranscode() charcode_error::dirty errors are "
                       "not handled"
                    << frg::endlog;
                break;
            }
            case mlibc::charcode_error::output_overflow: {
                mlibc::panicLogger()
                    << "decode_wtranscode() charcode_error::output_overflow "
...
size_t
mbstowcs(wchar_t *__restrict wcs, const char *__restrict mbs, size_t wc_limit) {
    auto                        cc = mlibc::current_charcode();
    ...
    if (auto e = cc->decode_wtranscode(nseq, wseq, st);
        e != mlibc::charcode_error::null) {
        __ensure(!"decode_wtranscode() errors are not handled");
        __builtin_unreachable();
    } else {
...
size_t wcstombs(char *__restrict mb_string,
                const wchar_t *__restrict wc_string,
                size_t max_size) {
...
void free(void *ptr) {
    ...
    if (mlibc::globalConfig().debugMalloc) {
        mlibc::infoLogger()
            << "mlibc (PID ?): free() on " << ptr << frg::endlog;
        if ((uintptr_t)ptr & 1)
...
void *malloc(size_t size) {
    ...
    if (mlibc::globalConfig().debugMalloc)
        mlibc::infoLogger()
            << "mlibc (PID ?): malloc() returns " << nptr << frg::endlog;
    return nptr;
}
...
void *realloc(void *ptr, size_t size) {
    ...
    // TODO: Print PID only if POSIX option is enabled.
    if (mlibc::globalConfig().debugMalloc)
        mlibc::infoLogger() << "mlibc (PID ?): realloc() on " << ptr
                            << " returns " << nptr << frg::endlog;
    return nptr;
}
...
int posix_memalign(void **out, size_t align, size_t size) {
    ...
    if (!p)
        return ENOMEM;
    // Hope that the alignment was respected. This works on the current
    // allocator.
